...
# ============================================================
# COMPOSITE TAB (fusion alignment logic)
# ============================================================
with tab_comb:
...
# ============================================================
# FUTURES CHART TAB  (same as earlier add-on)
# ============================================================
with tab_fut_chart:
    ...
    fig=go.Figure()
    fig.add_trace(go.Bar(x=df_sum["time"],y=df_sum["Δ Volume"],name="Δ Volume",marker_color="orange",yaxis="y2"))
...
with tab_opt_chart:
    ...
    files=st.file_uploader("Upload Option CSVs (_DDMMYYYY_HHMMSS.csv)",accept_multiple_files=True,key="opt_chart_up")
    if not files:
        st.info("Upload option CSVs to plot.")
        st.stop()
    def parse(name):
...
st.caption("✅ Fusion engine combines Futures and Options bias + momentum for clear directional signals.")
