...
with tab_fut:
    ...
    if not fut_files:
        ...
        fig.update_layout(title="Rolling Δ Metrics", height=400)
...
with tab_opt:
    ...
    opt_files = st.file_uploader("Upload Option‑Chain CSVs", type="csv", accept_multiple_files=True)
    df = pd.DataFrame()
...
with tab_comb:
    ...
    if fut_df.empty or df.empty or combined_fut.empty or opt_feat.empty:
    ...
    fut_m["time_key"] = pd.to_datetime(fut_m["time"], errors="coerce").dt.tz_localize(None).astype("datetime64[ns]")
    opt_m["time_key"] = pd.to_datetime(opt_m["timestamp"], errors="coerce").dt.tz_localize(None).astype("datetime64[ns]")
    ...
    def comp_sig(r):
        ...
        if f.startswith("🔴") and b == "bullish": return "🟣 Futures↓ / Opts Bullish (Reversal Watch)"
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=merged["time_key"], y=merged["Momentum_Score"], name="Momentum Score", marker_color="purple"))
    st.plotly_chart(fig, use_container_width=True)

    st.download_button("⬇️ Download Composite Signals",
                       merged[cols_to_show].to_csv(index=False).encode("utf‑8"),
                       "composite_signals.csv", "text/csv")

# ============================================================
# FUTURES CHART TAB (uses fut_df)
# ============================================================
with tab_fut_chart:
    st.subheader("📉 Futures Chart – Δ Volume vs Last Price (from upload above)")
    if fut_df.empty:
        st.info("Upload Futures CSVs in the first tab to see this chart.")
    else:
        df_sum = fut_df.sort_values("timestamp")
        df_sum["Δ Volume"] = df_sum["volume"].diff()
        df_sum["Δ Price"] = df_sum["lastPrice"].diff()
        axis_type = st.radio("Y‑axis scale", ["linear", "log"], horizontal=True)
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df_sum["timestamp"], y=df_sum["Δ Volume"],
                             name="Δ Volume", marker_color="orange", yaxis="y2"))
        fig.add_trace(go.Scatter(x=df_sum["timestamp"], y=df_sum["lastPrice"],
        ...
        fig.update_layout(height=600,
                          ...
                          title="Δ Volume vs Last Price", hovermode="x unified")
        st.plotly_chart(fig, use_container_width=True)

# ============================================================
# OPTIONS CHART TAB (uses df)
# ============================================================
with tab_opt_chart:
    st.subheader("📈 Options Chart Viewer – Δ Metrics per Strike (from upload above)")
    if df.empty:
        st.info("Upload Option CSVs in the Options tab to see this chart.")
    else:
        strikes = sorted(pd.to_numeric(df.get("CE_strikePrice", df.get("PE_strikePrice", pd.Series())),
                                       errors="coerce").dropna().unique())
        if not strikes:
            st.warning("No strike data found.")
        else:
            ...
            if col not in df:
                ...
                if sub.empty:
                    st.warning("No data for that strike.")
                else:
                    fig = px.line(sub, x="timestamp", y=col, markers=True,
                                  title=f"{opt} {metric} for Strike {strike}")
                    st.plotly_chart(fig, use_container_width=True)

st.caption("✅ Single‑upload engine — shared data powering analytics + charts for both Futures and Options.")
