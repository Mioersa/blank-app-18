...
with tab_fut:
    ...
    if not fut_files:
        ...
        fut_df, vol_df, oi_df, turn_df, combined_fut = [pd.DataFrame()] * 5
    else:
        dfs, labels, times = [], [], []
...
with tab_comb:
    ...
    if combined_fut.empty or opt_feat.empty:
        st.warning("Upload both Futures and Options files.")
        st.stop()

    # merge on nearest time
    fut_m = combined_fut.copy()
    opt_m = opt_feat.copy()
    opt_m["time"] = opt_m["timestamp"].dt.strftime("%H:%M")
    merged = pd.merge_asof(
        fut_m.sort_values("time"), opt_m.sort_values("time"),
        by=None, left_on="time", right_on="time", direction="nearest"
    )
    ...
    cols_to_show=["time","Overall_Fut_Signal","bias","Opt_Signal",
                  "Momentum_Score","Composite_Signal"]
    st.dataframe(merged[cols_to_show])

    st.subheader("Composite Momentum Chart")
    fig=go.Figure()
    fig.add_trace(go.Bar(x=merged["time"],y=merged["Momentum_Score"],
...
st.caption("Fusion engine: Futures × Options alignment + momentum intensity → directional clarity.")
